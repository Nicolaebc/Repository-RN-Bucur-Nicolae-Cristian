...
st.set_page_config(page_title="Intersectie Inteligenta - Machine Learning", layout="wide")
...
# Titlu si Sidebar
st.title("Sistem Inteligent de Control al Traficului")
st.caption(f"Model activ: `{os.path.basename(loaded_path)}`")
...
if sirena:
    final_decision = 0
    decision_label = "URGENTA"
    reason = "Am detectat vehicul interventie."
    bg_color = "#d9534f"
elif confidenta < 60.0:
    final_decision = -1 
    decision_label = "INCERTITUDINE - PRUDENTA"
    reason = f"Incredere scazuta ({confidenta:.1f}%). Modelul ezita intre stari."
    bg_color = "#f0ad4e" 
else:
...
st.caption(f"Audit activat: 'blackbox.log'. Total masini procesate: {st.session_state.total_cars}")
